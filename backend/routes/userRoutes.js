...
router.get('/auth/google/callback', 
  passport.authenticate('google', { failureRedirect: 'http://http://localhost:3000/Login', session: false }), 
  (req, res) => {
...
router.get('/auth/success', (req, res) => {
  ...
  return res.redirect(`http://http://localhost:3000/google-auth?token=${token}&role=${role}&name=${name}`);
});
