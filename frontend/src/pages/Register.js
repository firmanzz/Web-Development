...
const Register = () => {
  ...
  const handleDeleteUnverified = useCallback(async () => {
    if (isRegistered && email) {
      try {
        const response = await fetch('http://http://localhost:5000/api/delete-unverified', {
          method: 'POST',
          headers: {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await fetch('http://http://localhost:5000/api/register', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleVerify = async (e) => {
    ...
    try {
      const response = await fetch('http://http://localhost:5000/api/verify-email', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
