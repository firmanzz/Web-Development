...
const MovieListCMS = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        const response = await fetch("http://master-movie-cms.vercel.app/api/movies", {
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = Cookies.get("token"); // Ambil token dari cookies
      const response = await fetch(`http://master-movie-cms.vercel.app/api/movies/${id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`, // Tambahkan token ke header Authorization
      },
  ...
  const handleApproveToggle = async () => {
    ...
    try {
      ...
      `http://master-movie-cms.vercel.app/api/movies/${selectedMovie.id}`,
      {
        method: "PUT",
        headers: {
