...
const Directors = () => {
  ...
  useEffect(() => {
    const fetchDirectors = async () => {
      try {
        ...
        const response = await fetch("http://http://localhost:5000/api/directors", {
          headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      let response;
      if (selectedDirector) {
      response = await fetch(`http://http://localhost:5000/api/directors/${selectedDirector.id}`, {
        method: "PUT",
        headers: {
      ...
      response = await fetch("http://http://localhost:5000/api/addDirector", {
        method: "POST",
        headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`, // Tambahkan token ke header Authorization
  ...
  const handleDeleteDirector = async (id) => {
    try {
      const token = Cookies.get("token"); // Ambil token dari cookies
      const response = await fetch(`http://http://localhost:5000/api/directors/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`, // Tambahkan token ke header Authorization
        },
