...
const Actors = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      response = await fetch(
        `https://master-movie-cms.vercel.app/api/actors/${selectedActor.id}`,
        {
        method: "PUT",
        headers: {
      ...
      response = await fetch("https://master-movie-cms.vercel.app/api/addActor", {
        method: "POST",
        headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`, // Tambahkan token ke header Authorization
  ...
  const handleDeleteActor = async (id) => {
    try {
      const token = Cookies.get("token"); // Ambil token dari cookies
      const response = await fetch(`https://master-movie-cms.vercel.app/api/actors/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`, // Tambahkan token ke header Authorization
        },
