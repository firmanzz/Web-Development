...
const Users = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await fetch("http://http://localhost:5000/api/users", {
          headers: {
  ...
  const handleSuspendToggle = async () => {
    ...
    try {
      ...
      await fetch(`http://http://localhost:5000/api/users/${selectedUser.id}`, {
      method: "PUT",
      headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = Cookies.get("token"); // Ambil token dari cookies
      const response = await fetch(`http://http://localhost:5000/api/users/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`, // Tambahkan token ke header Authorization
